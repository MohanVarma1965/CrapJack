    ...
    if (calculatedScore[0] <= 21 && calculatedScore[1] > 21 || calculatedScore[0] <= 21 && calculatedScore[1] <= 21 && calculatedScore[0] > calculatedScore[1] || !calculatedScore[0] > 21) {
      ...
      return (
        <div>
          <div className="congrats">
            <div> Dealer Wins </div>
            {calculatedScore[0] > 21 ? <div>You are busted</div> : <div> Your score {calculatedScore[0]} </div>}
            {calculatedScore[1] > 21 ? <div> Dealer has the advantage </div> : <div> Dealer score {calculatedScore[1]}</div>}
          </div>
        </div>
      );
    }
